...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=//
/*
*  1. {} objects are implicitly ANDs.  An empty object will keep truthy items.
*  2. [] arrays are implicitly ORs.  An empty array will keep falsey items.  
...
*		This only works at the top level/initial call, like SQL 'where true'.
*  4. false will keep no results.  Provided as a fallback for degenerate cases.
*		This only works at the top level/initial call, like SQL 'where false'.
...
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=//
	var COUNT = 'count';
	// Not 'keeping' anything if nothing or null is passed.
	var keeping = !(this instanceof window.constructor || this === null);
	if( keeping )
	{
		var operator = typeof( this ) !== 'boolean' && DataFilter.Keys.OP in this && this[ DataFilter.Keys.OP ] || '===';
		...
		if( this instanceof Boolean || typeof( this ) === 'boolean' )
		{
		...
		else if( this instanceof Array )
		{
			...
			{
				// A null object is kept by an empty array, but discarded by an empty object (because it is "falsey");
				//	a non-null object is discarded by an empty array, but kept by an empty object (because it is "truthy").
		...
		else if( this !== null && is_plain_object( this ))
		{
			...
			if( sub_clause_keys.length > 0 )
			{
				keeping = sub_clause_keys.every( function outer_and_clause( sub_key, s, sub_keys )
				{
					...
					if( sub_key in reservedKeyByValue )
					{
						switch( sub_key )
						{
							...
							case DataFilter.Keys.IS:
								...
								if((each !== null && is_plain_object( this[ sub_key ])) || (this[ sub_key ] instanceof Array))
								{
									var currData = ((each instanceof Array)?(each):([each]));
									var is_clause_keeper = every_or_some( this[ sub_key ]);
									sub_keeping = currData[ is_clause_keeper ]( DataFilter.resultsWhere, this[ sub_key ]);
								}
								/*if( each !== null && is_plain_object( this[ sub_key ]))
								{
									var currData = ((each instanceof Array)?(each):([each]));
									var is_clause_keys = Object.keys( this[ sub_key ])
										.filter( not_op_key );
									sub_keeping = is_clause_keys.every( function( isClause, i, isClauses )
									{
										var sub_sub_keeping = currData.some( DataFilter.resultsWhere, isClause );
										return( sub_sub_keeping );
									}, this[ sub_key ]);
								}
								else if( this[ sub_key ] instanceof Array )
								{
									var currData = ((each instanceof Array)?(each):([each]));
									sub_keeping = this[ sub_key ].some( function( isClause, i, isClauses )
									{
										var sub_sub_keeping = currData.some( DataFilter.resultsWhere, isClause );
										return( sub_sub_keeping );
									}, this[ sub_key ]);
								...
								{sub_keeping = comparator( each, this[ sub_key ]);}
								break;
							case DataFilter.Keys.NOT:
								var currData = ((each instanceof Array)?(each):([each]));
								var not_clause_keeper = every_or_some( this[ sub_key ]);
								sub_keeping = !currData[ not_clause_keeper ]( DataFilter.resultsWhere, this[ sub_key ]);
								break;
							case DataFilter.Keys.OR:
								if( this[ sub_key ] !== null && is_plain_object( this[ sub_key ]))
								{
									...
									sub_keeping = exp_clause_keys.some( function explicit_or_clause( exp_clause_key, eck, exp_clause_keys )
									{
										...
										{sub_sub_keeping = ('length' in every && comparator( every.length, window.parseInt( this[ sub_key ], 10 )));}
										else if( exp_clause_key === 'length' )
										{sub_sub_keeping = (!!each && each.hasOwnProperty( 'length' ) && comparator( each.length, window.parseInt( this[ sub_key ], 10 )));}
										else
										{
							...
							case DataFilter.Keys.SUB_CLAUSE:
								var filtered = every.filter( DataFilter.resultsWhere, this[ sub_key ]);
					...
					{sub_keeping = ('length' in every && comparator( every.length, window.parseInt( this[ sub_key ], 10 )));}
					else if( sub_key === 'length' )
					{sub_keeping = (!!each && each.hasOwnProperty( 'length' ) && comparator( each.length, window.parseInt( this[ sub_key ], 10 )));}
					else if( !!each )
					{
						if( sub_key in each )
						{
							...
							else if( this[ sub_key ] !== null && is_plain_object( this[ sub_key ]) /*&& is_plain_object( each[ sub_key ])*/)
							{
								...
								else
								{
									// A null object is kept by an empty array, but discarded by an empty object (because it is "falsey");
									//	a non-null object is discarded by an empty array, but kept by an empty object (because it is "truthy").
									sub_keeping = ((this[ sub_key ] instanceof Array && this[ sub_key ].length == 0)
										?(!each[ sub_key ])
										:(!!each[ sub_key ]));
								}
							}
